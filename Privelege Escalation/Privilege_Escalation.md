...
- ``` bash find / -type f -perm -04000 -ls 2>/dev/null # (***oldukça öenmli)```
- ``` bash find / -writable -type d 2>/dev/null ```
- ``` bash find . -name flag1.txt # : geçerli dizinde “flag1.txt” adlı dosyayı bulun```
- ``` bash find /home -name flag1.txt #: /home dizinindeki “flag1.txt” dosya adlarını bulun```
- ``` bash find / -type d -name config #: “/” altında config adlı dizini bulun```
- ``` bash find / -type f -perm 0777 # : 777 izinlerine sahip dosyaları bul (tüm kullanıcılar tarafından okunabilir, yazılabilir ve çalıştırılabilir dosyalar)```
- ``` bash find / -perm a=x # : çalıştırılabilir dosyaları bul```
- ``` bash find /home -user frank #: “frank” kullanıcısına ait tüm dosyaları “/home” altında bul```
- ``` bash find / -mtime 10 # : Son 10 günde değiştirilen dosyaları bul```
- ``` bash find / -atime 10 #: Son 10 günde erişilen dosyaları bul```
- ``` bash find / -cmin -60 #: Son bir saat (60 dakika) içinde değiştirilen dosyaları bul```
- ``` bash find / -amin -60 #: Son bir saat (60 dakika) içinde erişilen dosyaları bul```
- ``` bash find / -size 50M #: 50 MB boyutundaki dosyaları bul ```
